    ...
    def prm_ranking(self, K1: float = 1.2, K2: float = 500, B: float = 0.85, top_k_ratio: float = 0.1):
        ...
        for k1 in k1_values:
            for k2 in k2_values:
                ...
                total_score = 0
                document_weighting = self.bm25_ranking(using_relevance=True, K1=K1, K2=K2, B=B,
                                                       R=len(sorted_document_weighting), ri=total_collection_terms)
        ...
        document_weighting = self.bm25_ranking(using_relevance=True, K1=local_best_k1, K2=local_best_k2, B=B,
                                               R=len(sorted_document_weighting), ri=total_collection_terms)
